...
from admin_panel.telebot.admin import bot_admin
...
@admin.register(ConfigRow, site=bot_admin)
class ConfigRowAdmin(ImportExportModelAdmin):
    form = ConfigRowForm
    formats = [formats.base_formats.JSON]
    resource_class = ConfigRowResource
    list_display = ("name", "value_short", "description", "topic", "tags", "last_read", "last_set")
    list_filter = (
        "topic",
        TagsListFilter,
    )
